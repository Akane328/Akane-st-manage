...
_restore_latest_backup() {
    info "正在查找最新的备份..."
    local latest_backup=$(ls -dt "$BACKUP_PARENT_DIR"/sillytavern_backup_* 2>/dev/null | head -n 1)
    if [[ -z "$latest_backup" ]]; then
        warn "未找到任何备份，无法自动恢复数据。将使用更新后的默认数据。"
        return 1
    fi
    info "找到最新备份: $(basename "$latest_backup")"
    info "正在从备份中恢复 'public' 和 'data' 目录..."
    rm -rf "$ST_DIR/public" "$ST_DIR/data"
    # 确保备份中的目录存在再复制
    if [ -d "$latest_backup/public" ]; then 
        cp -rp "$latest_backup/public" "$ST_DIR/"
    else
        warn "备份中未找到 'public' 目录，跳过。"
    fi
    if [ -d "$latest_backup/data" ]; then 
        cp -rp "$latest_backup/data" "$ST_DIR/"
    else
        warn "备份中未找到 'data' 目录，跳过。"
    fi
    success "数据恢复完成。"
    return 0
}
...
manage_sillytavern() {
    ...
    if [[ "$local_ver" != "未安装" ]]; then
        ...
        if ! npm install --omit=dev; then err "NPM依赖更新失败。"; return 1; fi
...
uninstall_sillytavern() {
    check_and_install_deps "jq" || return 1; 
        if [[ "$(get_local_st_ver)" == "未安装" ]]; then
        err "请先安装SillyTavern。"
        read -n1 -s -r -p "按任意键确认并返回..."
        return
    fi
...
start_menu() {
    ...
    clear; echo -e "${CYAN}--- 启动管理（通过Screen） ---${NC}"
    echo -e "  ${GREEN}1)${NC} 前台启动\n  ${GREEN}2)${NC} 后台启动\n  ${GREEN}3)${NC} 查看日志\n  ${RED}4)${NC} 停止酒馆\n"
    
    if [[ -z "$TERMUX_VERSION" ]]; then
        echo -e "${CYAN}--- 开机自启（通过Systemd） ---${NC}"
        local service_enabled=false
        [[ -f "$SERVICE_FILE" ]] && systemctl is-enabled --quiet "$SERVICE_NAME" 2>/dev/null && service_enabled=true
...
# [新增] 插件安装通用函数
_install_plugin() {
    local plugin_display_name="$1"
    local domestic_url="$2"
    local foreign_url="$3"
    
    local repo_name=$(basename "$foreign_url" .git)
    local extensions_dir="$ST_DIR/data/default-user/extensions"
    local target_dir="$extensions_dir/$repo_name"

    info "正在准备安装插件: ${plugin_display_name}"

    if [ -d "$target_dir" ]; then
        warn "插件目录 '$repo_name' 已存在。"
        read -rp "是否要删除并重新安装? (Y/n): " yn
        if [[ "$yn" =~ ^[Nn]$ ]]; then
            info "安装已取消。"
            return 1
        fi
    ...
    info "确保插件目录存在: $extensions_dir"
    mkdir -p "$extensions_dir"

    local urls=()
    if [[ "$PROXY_ENABLED" == true ]]; then
        [[ -n "$domestic_url" ]] && urls+=("$domestic_url")
        [[ -n "$foreign_url" ]] && urls+=("$foreign_url")
    else
        [[ -n "$foreign_url" ]] && urls+=("$foreign_url")
        [[ -n "$domestic_url" ]] && urls+=("$domestic_url")
    fi
    ...
    local cloned=false
    for url in "${urls[@]}"; do
        info "正在尝试从源克隆: $(echo "$url" | cut -d'/' -f3)..."
        if git clone --depth 1 "$url" "$target_dir"; then
            success "插件 '${plugin_display_name}' 安装成功！"
            cloned=true
            break
        else
            warn "从 $(echo "$url" | cut -d'/' -f3) 克隆失败，尝试下一个源..."
        fi
    done

    if [[ "$cloned" == false ]]; then
        err "安装插件 '${plugin_display_name}' 失败，所有源都无法访问。"
        return 1
    fi
    return 0
}
...
plugin_menu() {
    ...
    while true; do
        local extensions_dir="$ST_DIR/data/default-user/extensions"
        local mem_plugin_status; local helper_plugin_status
        if [ -d "$extensions_dir/st-memory-enhancement" ]; then mem_plugin_status="${GREEN}(已安装)${NC}"; else mem_plugin_status="${YELLOW}(未安装)${NC}"; fi
        if [ -d "$extensions_dir/JS-Slash-Runner" ]; then helper_plugin_status="${GREEN}(已安装)${NC}"; else helper_plugin_status="${YELLOW}(未安装)${NC}"; fi
 
        clear; echo -e "${CYAN}--- 插件管理 ---${NC}"
        echo -e "${CYAN}--------------------------------------------------${NC}"
        echo -e "  ${GREEN}1)${NC} 安装 酒馆记忆插件 ${mem_plugin_status}"
        echo -e "  ${GREEN}2)${NC} 安装 酒馆助手插件 ${helper_plugin_status}\n"
        echo -e "  ${CYAN}3)${NC} 一键安装所有推荐插件"
        echo -e "  ${WHITE}0)${NC} 返回主菜单"
        read -rp "请选择操作 [0-3]: " choice
        
        case "$choice" in
            1) _install_plugin "酒馆记忆插件" "https://gitee.com/muyoou/st-memory-enhancement.git" "https://github.com/muyoou/st-memory-enhancement.git" ;;
            2) _install_plugin "酒馆助手插件" "https://gitlab.com/novi028/JS-Slash-Runner.git" "https://github.com/N0VI028/JS-Slash-Runner.git" ;;
            3) 
                info "--- 开始一键安装所有插件 ---"
                _install_plugin "酒馆记忆插件" "https://gitee.com/muyoou/st-memory-enhancement.git" "https://github.com/muyoou/st-memory-enhancement.git"
                echo
                _install_plugin "酒馆助手插件" "https://gitlab.com/novi028/JS-Slash-Runner.git" "https://github.com/N0VI028/JS-Slash-Runner.git"
                success "--- 所有插件安装任务已执行完毕 ---"
                ;;
            0) return ;;
            *) err "无效选项" ;;
        esac
...
main_menu() {
    while true; do
        ...
        echo -e "${CYAN}--------------------------------------------------${NC}"
        echo -e "  ${GREEN}1)${NC} 安装/更新酒馆"
        echo -e "  ${GREEN}2)${NC} ${start_restart_text}"
        echo -e "  ${GREEN}3)${NC} 启动管理"
        echo -e "  ${GREEN}4)${NC} 配置管理"
        echo -e "  ${GREEN}5)${NC} 备份管理"
        echo -e "  ${GREEN}6)${NC} 插件管理"
        echo -e "  ${RED}7)${NC} 卸载酒馆\n"
        echo -e "  ${CYAN}9)${NC} 检查脚本更新"
        echo -e "  ${WHITE}0)${NC} 退出\n${CYAN}==================================================${NC}"
        read -rp "请选择操作 [0-9]: " opt
        case "$opt" in
            1) manage_sillytavern ;;
            2) manage_start_or_restart ;;
            3) start_menu ;;
            4) config_menu ;;
            5) backup_menu ;;
            6) plugin_menu ;; 
            7) uninstall_sillytavern ;;
            9) update_script ;;
            0) exit 0 ;;
            *) err "无效选项，请重试。" ;;
        esac
        if [[ "$opt" != "0" && "$opt" != "2" && "$opt" != "3" && "$opt" != "4" && "$opt" != "7" ]]; then
