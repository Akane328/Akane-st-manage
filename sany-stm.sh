...
check_and_install_deps() {
    ...
    if [[ -n "$TERMUX_VERSION" ]]; then
        ...
        while fuser "$lock_file" >/dev/null 2>&1 || fuser "$lock_file_frontend" >/dev/null 2>&1; do
            if (( count >= max_wait_seconds )); then
                err "Termux 包管理器(apt/pkg)已被另一个进程锁定超过 ${max_wait_seconds} 秒。"
                err "请关闭其他正在使用 'pkg' 或 'apt' 的终端会话，或运行 'killall -9 pkg apt' 后重试。"
                return 1
            fi
            warn "检测到另一个包管理进程正在运行，等待其完成... (${count}s / ${max_wait_seconds}s)"
            sleep 1
            ((count++))
        done
        success "包管理器已就绪。"
 
        if [[ " ${required_deps[*]} " =~ " nodejs " ]]; then
            info "检测到Termux环境，正在升级所有已安装的软件包..."
            pkg update -y || { err "运行 'pkg update' 失败！"; return 1; }
            pkg upgrade -y -o Dpkg::Options::="--force-confnew" || { err "运行 'pkg upgrade' 失败！"; return 1; }
            success "Termux 软件包升级完成。"
        fi
    fi
    ...
    if [[ -n "$TERMUX_VERSION" ]]; then
        ...
        for dep in "${required_deps[@]}"; do
            if ! command -v "$dep" >/dev/null; then
                err "依赖 '$dep' 未找到。无法识别您的包管理器，请手动安装。"
                return 1
            fi
        done
        success "所有依赖项均已就绪。"
    ...
    for dep in "${required_deps[@]}"; do
        if [[ "$dep" == "nodejs" ]] && command -v "node" >/dev/null; then continue; fi
        if [[ "$dep" == "npm" ]] && command -v "npm" >/dev/null; then continue; fi
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        ...
        for dep in "${missing_deps[@]}"; do
             case "$pkg_manager" in
                "apt-get")
                    if [[ "$dep" == "nodejs" || "$dep" == "npm" ]]; then
                        if ! [[ " ${packages_to_install[*]} " =~ " nodejs " ]]; then packages_to_install+=("nodejs" "npm"); fi
                ...
                "pkg") 
                    if [[ "$dep" == "npm" ]]; then continue; fi
                    if [[ "$dep" == "dnsutils" ]]; then packages_to_install+=("dnsutils");
                    else packages_to_install+=("$dep"); fi ;;
             esac
        done
 
        if [ ${#packages_to_install[@]} -eq 0 ]; then
             success "依赖项将在其他软件包安装时一并解决。"
        else
...
manage_sillytavern() {
    check_and_install_deps "git" "curl" "nodejs" "npm" "jq" || { err "核心依赖检查或安装失败，操作中止。"; return 1; }
...
handle_proxy_logic() {
    ...
    if [[ "$country" == "CN" ]]; then
        info "检测到您可能位于中国大陆(CN)。推荐开启代理以加速GitHub访问。"; read -rp "是否启用GitHub加速代理? [Y/n]: " yn
        if [[ ! "$yn" =~ ^[Nn]$ ]]; then PROXY_ENABLED=true; else PROXY_ENABLED=false; fi
    else
        info "检测到您位于海外地区 (${country:-未知})。"; read -rp "您是否需要启用GitHub加速代理? [y/N]: " yn
    ...
    PROXY_CONFIGURED_MANUALLY=true; [[ "$PROXY_ENABLED" == true ]] && info "当前GitHub访问模式: ${GREEN}代理模式${NC}" || info "当前GitHub访问模式: ${YELLOW}直连模式${NC}"
}
...
# ... (rotate_backups, _create_backup_instance, uninstall_sillytavern, backup_menu 均无逻辑错误，保持原样) ...
rotate_backups() {
    info "正在检查并轮替备份..."
            ...
            case "$choice" in
                5)
                    if [[ "$service_enabled" == true ]]; then
                        ...
                        info "正在设置开机自启并启动服务..."
                        local node_path=$(command -v node); [[ -z "$node_path" ]] && { err "无法找到 'node' 命令。"; return 1; }
...
# [修复] 提供了更健壮的PATH环境变量，以提高在不同系统上的兼容性。
...
main_menu() {
    while true; do
        check_and_install_deps "jq" >/dev/null 2>&1 &
